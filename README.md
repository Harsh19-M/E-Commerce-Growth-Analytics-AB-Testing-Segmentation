...
- **Python-Based Funnel Analysis, Segmentation & Feature Engineering:** Extended the SQL output in Python by constructing a full-funnel framework (Visit → View → Add-to-Cart → Purchase) and engineering 7 additional analytical columns, including conversion rates, lift metrics, engagement tiers, and purchase-intent segments. Used explicit if / elif / else logic to segment users by Engagement Level and Purchase Intent, ensuring transparent, rule-based classification rather than black-box methods. Validated experiment balance, exposure integrity, and metric consistency before final analysis.

...
## Key Highlights 
* **PostgreSQL Data Aggregation & Cleaning:** Executed ~10 exploratory SQL queries to validate experiment exposure, session logic, and event tracking. Aggregated 80K sessions, 8K users, 30K orders, and 8 experiments into a 1,563-row analysis-ready table using `multi-step CTEs` with `window functions`, filtered counts `(COUNT(*) FILTER)`, and distinct aggregations. Applied null handling `(COALESCE)` and boolean event logic to accurately capture clicks, add-to-cart actions, purchases, and orders, ensuring correct session-level aggregation for reliable downstream analysis.
* **Python Funnel Analysis, Segmentation & Feature Engineering:** Extended the SQL output in Python by constructing a behaviorally ordered full-funnel framework  (Visit → View → ATC → Purchase) and engineered behavioral features (conversion, lift, engagement, intent). Applied explicit rules using ``if / elif / else`` statements to segment users by ``Engagement Level`` and ``Purchase Intent``, ensuring transparent, rule-based, and explainable user classification. Created drop-off tables while validating experiment balance, metric consistency, and exposure integrity.
* **EDA Experiment Insights & Data Re-Loading:** Identified ``Variant B as the top performer (5/8 experiments, up to +211% lift)`` and showed that completed purchase—not CTR or ATC—was the correct decision metric. Mapped lift to specific UI elements (``Trust Badges``, ``Urgency Banners``) and reloaded enriched tables into PostgreSQL DB for BI and reuse.
* **Statistical Testing & Validation:** Applied ``one-sided proportion z-tests``, ``chi-square independence tests``, ``ANOVA``, and ``logistic regression models`` to validate experiment results, segment effects, and UI performance. Controlled for confounders and confirmed Variant B as the consistently significant conversion driver.
* **Business Recommendations:** Translated analysis into scalable growth strategies, recommending targeted rollout of winning UI variants to mid-funnel users to reduce drop-offs and maximize ROI.
